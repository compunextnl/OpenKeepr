...
TRANSLATIONS: dict[str, dict[str, str]] = {
    ...
    # --- SEO titles & descriptions (v1.3 — tuned to 50-60 / 110-160 chars) ---
    "Zero-knowledge encrypted message sharing": {
        "nl": "Zero-knowledge versleuteld berichten delen",
        "fr": "Partage de messages chiffrés zero-knowledge",
        "de": "Zero-Knowledge-verschlüsseltes Nachrichten-Sharing",
        "es": "Mensajería cifrada zero-knowledge",
        "it": "Condivisione messaggi crittografati zero-knowledge",
    },
    "Share passwords, recovery phrases or sensitive notes through a one-time, end-to-end encrypted link. Zero-knowledge, self-destructing.": {
        "nl": "Deel wachtwoorden, recovery phrases of gevoelige notities via een eenmalige, end-to-end versleutelde link. Zero-knowledge, zelfvernietigend.",
        "fr": "Partagez mots de passe, phrases de récupération ou notes sensibles via un lien unique chiffré de bout en bout. Zero-knowledge, autodestructeur.",
        "de": "Teile Passwörter, Recovery-Phrasen oder sensible Notizen über einen einmaligen, Ende-zu-Ende verschlüsselten Link. Zero-Knowledge, selbstzerstörend.",
        "es": "Comparte contraseñas, frases de recuperación o notas sensibles mediante un enlace único cifrado de extremo a extremo. Zero-knowledge, autodestructivo.",
        "it": "Condividi password, recovery phrase o note sensibili tramite un link monouso crittografato end-to-end. Zero-knowledge, autodistruggente.",
    },
    "zero-knowledge encrypted messaging": {
        "nl": "zero-knowledge versleutelde berichten",
        "fr": "messagerie chiffrée zero-knowledge",
        "de": "Zero-Knowledge verschlüsselte Nachrichten",
        "es": "mensajería cifrada zero-knowledge",
        "it": "messaggi crittografati zero-knowledge",
    },
    "How %(name)s keeps shared secrets safe: AES-256-GCM encryption in your browser, recipient verification and self-destructing messages.": {
        "nl": "Hoe %(name)s gedeelde informatie veilig houdt: AES-256-GCM versleuteling in je browser, ontvangerverificatie en zelfvernietigende berichten.",
        "fr": "Comment %(name)s protège les informations partagées : chiffrement AES-256-GCM dans votre navigateur, vérification du destinataire, messages autodestructeurs.",
        "de": "Wie %(name)s geteilte Inhalte schützt: AES-256-GCM-Verschlüsselung im Browser, Empfängerverifizierung, selbstzerstörende Nachrichten.",
        "es": "Cómo %(name)s protege la información compartida: cifrado AES-256-GCM en tu navegador, verificación del destinatario, mensajes autodestructivos.",
        "it": "Come %(name)s protegge le informazioni condivise: crittografia AES-256-GCM nel browser, verifica del destinatario, messaggi autodistruggenti.",
    },
    "programmatic encrypted messaging": {
        "nl": "programmatische versleutelde berichten",
        "fr": "messagerie chiffrée par programmation",
        "de": "programmatische verschlüsselte Nachrichten",
        "es": "mensajería cifrada programática",
        "it": "messaggi crittografati programmatici",
    },
    "REST API for %(name)s: programmatic zero-knowledge encrypted messaging with scoped API keys, attachment uploads and rate limiting.": {
        "nl": "REST API voor %(name)s: programmatische zero-knowledge versleutelde berichten met gescopete API keys, bijlage-uploads en rate limiting.",
        "fr": "API REST pour %(name)s : messagerie chiffrée zero-knowledge par programmation, avec clés API à portée limitée, upload de pièces jointes et limitation de débit.",
        "de": "REST-API für %(name)s: programmatische Zero-Knowledge-verschlüsselte Nachrichten mit scoped API-Schlüsseln, Anhang-Uploads und Rate-Limiting.",
        "es": "API REST para %(name)s: mensajería cifrada zero-knowledge programática con claves API con ámbitos limitados, subida de adjuntos y rate limiting.",
        "it": "API REST per %(name)s: messaggi crittografati zero-knowledge programmatici con chiavi API con scope, upload di allegati e rate limiting.",
    },
    "release notes — every change, fix and security update": {
        "nl": "release notes — elke wijziging, fix en beveiligingsupdate",
        "fr": "notes de version — chaque modification, correction et mise à jour de sécurité",
        "de": "Release Notes — jede Änderung, jeder Fix und jedes Sicherheitsupdate",
        "es": "notas de versión — cada cambio, corrección y actualización de seguridad",
        "it": "note di rilascio — ogni modifica, fix e aggiornamento di sicurezza",
    },
    "release notes — changes, fixes, security updates": {
        "nl": "release notes — wijzigingen, fixes, beveiligingsupdates",
        "fr": "notes de version — changements, correctifs, mises à jour de sécurité",
        "de": "Release Notes — Änderungen, Fixes, Sicherheitsupdates",
        "es": "notas de versión — cambios, correcciones, actualizaciones de seguridad",
        "it": "note di rilascio — modifiche, fix, aggiornamenti di sicurezza",
    },
    "Release notes for %(name)s — every change, bug fix and security update, version by version. Track new features and improvements.": {
        "nl": "Release notes voor %(name)s — elke wijziging, bugfix en beveiligingsupdate, versie voor versie. Volg nieuwe features en verbeteringen.",
        "fr": "Notes de version pour %(name)s — chaque modification, correction de bug et mise à jour de sécurité, version par version. Suivez les nouvelles fonctionnalités et améliorations.",
        "de": "Release Notes für %(name)s — jede Änderung, jeder Bugfix und jedes Sicherheitsupdate, Version für Version. Verfolge neue Funktionen und Verbesserungen.",
        "es": "Notas de versión de %(name)s — cada cambio, corrección de errores y actualización de seguridad, versión a versión. Sigue las nuevas funciones y mejoras.",
        "it": "Note di rilascio di %(name)s — ogni modifica, bug fix e aggiornamento di sicurezza, versione per versione. Segui le nuove funzionalità e i miglioramenti.",
    },
    "Privacy policy": {"nl": "Privacybeleid", "fr": "Politique de confidentialité", "de": "Datenschutzerklärung", "es": "Política de privacidad", "it": "Informativa sulla privacy"},
    "zero-knowledge messaging": {
        "nl": "zero-knowledge berichten",
        "fr": "messagerie zero-knowledge",
        "de": "Zero-Knowledge-Nachrichten",
        "es": "mensajería zero-knowledge",
        "it": "messaggi zero-knowledge",
    },
    "%(name)s privacy policy: we store only ciphertext and hashed metadata, never your messages or keys. How long anything is kept.": {
        "nl": "Privacybeleid van %(name)s: we slaan alleen ciphertext en gehashte metadata op, nooit je berichten of sleutels. Hoe lang we iets bewaren.",
        "fr": "Politique de confidentialité de %(name)s : nous stockons uniquement du chiffré et des métadonnées hachées, jamais vos messages ou clés. Durée de conservation.",
        "de": "Datenschutzerklärung von %(name)s: Wir speichern nur Ciphertext und gehashte Metadaten, niemals deine Nachrichten oder Schlüssel. Wie lange wir was aufbewahren.",
        "es": "Política de privacidad de %(name)s: almacenamos solo cifrado y metadatos con hash, nunca tus mensajes ni claves. Cuánto tiempo se conserva todo.",
        "it": "Informativa sulla privacy di %(name)s: memorizziamo solo ciphertext e metadati hash, mai i tuoi messaggi o le chiavi. Per quanto tempo conserviamo.",
    },
    "Cookie policy": {"nl": "Cookiebeleid", "fr": "Politique des cookies", "de": "Cookie-Richtlinie", "es": "Política de cookies", "it": "Informativa sui cookie"},
    "uses only essential cookies": {
        "nl": "gebruikt alleen essentiële cookies",
        "fr": "n'utilise que des cookies essentiels",
        "de": "verwendet nur essenzielle Cookies",
        "es": "usa solo cookies esenciales",
        "it": "usa solo cookie essenziali",
    },
    "Security policy": {"nl": "Beveiligingsbeleid", "fr": "Politique de sécurité", "de": "Sicherheitsrichtlinie", "es": "Política de seguridad", "it": "Policy di sicurezza"},
    "encryption and threat model": {
        "nl": "versleuteling en dreigingsmodel",
        "fr": "chiffrement et modèle de menace",
        "de": "Verschlüsselung und Bedrohungsmodell",
        "es": "cifrado y modelo de amenazas",
        "it": "crittografia e threat model",
    },
    "End-to-end encrypted, one-time secure message sharing. Send passwords, recovery phrases and notes that self-destruct after reading.": {
        "nl": "End-to-end versleutelde, eenmalige veilige berichten. Verstuur wachtwoorden, recovery phrases en notities die na lezen zelf vernietigd worden.",
        "fr": "Partage de messages sécurisé à usage unique, chiffré de bout en bout. Envoyez mots de passe, phrases de récupération et notes qui s'autodétruisent.",
        "de": "Ende-zu-Ende verschlüsseltes, einmaliges Teilen sicherer Nachrichten. Sende Passwörter, Recovery-Phrasen und Notizen, die sich selbst zerstören.",
        "es": "Mensajes seguros de un solo uso, cifrados de extremo a extremo. Envía contraseñas, frases de recuperación y notas que se autodestruyen.",
        "it": "Condivisione di messaggi sicuri monouso, crittografati end-to-end. Invia password, recovery phrase e note che si autodistruggono.",
    },

    # --- SEO meta descriptions (v1.3) ---
    "OpenKeepr — end-to-end encrypted, one-time secure message sharing. Send passwords, recovery phrases and sensitive notes that self-destruct after reading.": {
        "nl": "OpenKeepr — end-to-end versleutelde, eenmalige veilige berichten. Verstuur wachtwoorden, recovery phrases en gevoelige notities die na lezen vanzelf vernietigd worden.",
        "fr": "OpenKeepr — partage de messages sécurisé, chiffré de bout en bout et à usage unique. Envoyez mots de passe, phrases de récupération et notes sensibles qui s'autodétruisent après lecture.",
        "de": "OpenKeepr — Ende-zu-Ende verschlüsseltes, einmaliges Teilen sicherer Nachrichten. Sende Passwörter, Recovery-Phrasen und sensible Notizen, die sich nach dem Lesen selbst zerstören.",
        "es": "OpenKeepr — Mensajes seguros de un solo uso, cifrados de extremo a extremo. Envía contraseñas, frases de recuperación y notas sensibles que se autodestruyen tras leerse.",
        "it": "OpenKeepr — Condivisione di messaggi sicuri, monouso e crittografati end-to-end. Invia password, recovery phrase e note sensibili che si autodistruggono dopo la lettura.",
    },
    "Share passwords, recovery phrases or any sensitive note through a single one-time link. End-to-end encrypted in your browser — zero-knowledge, self-destructing, with optional e-mail verification.": {
        "nl": "Deel wachtwoorden, recovery phrases of andere gevoelige informatie via één eenmalige link. End-to-end versleuteld in je browser — zero-knowledge, zelfvernietigend, met optionele e-mailverificatie.",
        "fr": "Partagez mots de passe, phrases de récupération ou toute note sensible via un lien unique à usage unique. Chiffré de bout en bout dans votre navigateur — zero-knowledge, autodétruisant, avec vérification e-mail optionnelle.",
        "de": "Teile Passwörter, Recovery-Phrasen oder sensible Notizen über einen einmaligen Link. Ende-zu-Ende verschlüsselt im Browser — Zero-Knowledge, selbstzerstörend, mit optionaler E-Mail-Verifizierung.",
        "es": "Comparte contraseñas, frases de recuperación o cualquier nota sensible a través de un único enlace de un solo uso. Cifrado de extremo a extremo en tu navegador — zero-knowledge, autodestructivo, con verificación por correo opcional.",
        "it": "Condividi password, recovery phrase o qualsiasi nota sensibile tramite un link monouso. Crittografato end-to-end nel tuo browser — zero-knowledge, autodistruggente, con verifica e-mail opzionale.",
    },
    "Learn how %(name)s keeps shared secrets safe: AES-256-GCM encryption in your browser, recipient verification, automatic self-destruction, and a server that simply cannot read your messages.": {
        "nl": "Ontdek hoe %(name)s gedeelde informatie veilig houdt: AES-256-GCM versleuteling in je browser, ontvangerverificatie, automatische zelfvernietiging en een server die je berichten simpelweg niet kan lezen.",
        "fr": "Découvrez comment %(name)s protège les informations partagées : chiffrement AES-256-GCM dans votre navigateur, vérification du destinataire, autodestruction automatique et un serveur qui ne peut tout simplement pas lire vos messages.",
        "de": "Erfahre, wie %(name)s geteilte Inhalte schützt: AES-256-GCM-Verschlüsselung im Browser, Empfängerverifizierung, automatische Selbstzerstörung und ein Server, der deine Nachrichten schlicht nicht lesen kann.",
        "es": "Descubre cómo %(name)s protege la información compartida: cifrado AES-256-GCM en tu navegador, verificación del destinatario, autodestrucción automática y un servidor que sencillamente no puede leer tus mensajes.",
        "it": "Scopri come %(name)s tiene al sicuro le informazioni condivise: crittografia AES-256-GCM nel browser, verifica del destinatario, autodistruzione automatica e un server che semplicemente non può leggere i tuoi messaggi.",
    },
    "REST API for %(name)s: programmatic creation of zero-knowledge encrypted messages, attachment uploads and recipient management — with scoped, per-account API keys and rate limiting.": {
        "nl": "REST API voor %(name)s: programmatisch zero-knowledge versleutelde berichten maken, bijlagen uploaden en ontvangers beheren — met gescopete API keys per account en rate limiting.",
        "fr": "API REST de %(name)s : création programmatique de messages chiffrés zero-knowledge, téléchargement de pièces jointes et gestion des destinataires — avec des clés API à portée limitée par compte et de la limitation de débit.",
        "de": "REST-API für %(name)s: programmatisches Erstellen von Zero-Knowledge-verschlüsselten Nachrichten, Anhang-Uploads und Empfängerverwaltung — mit kontogebundenen, scoped API-Schlüsseln und Rate-Limiting.",
        "es": "API REST de %(name)s: creación programática de mensajes cifrados zero-knowledge, subida de adjuntos y gestión de destinatarios — con claves API con ámbitos limitados por cuenta y límite de tasa.",
        "it": "API REST di %(name)s: creazione programmatica di messaggi crittografati zero-knowledge, upload di allegati e gestione dei destinatari — con chiavi API con scope per account e rate limiting.",
    },
    "Release notes for %(name)s — every change, fix and security update, version by version.": {
        "nl": "Release notes voor %(name)s — elke wijziging, fix en beveiligingsupdate, versie voor versie.",
        "fr": "Notes de version pour %(name)s — chaque modification, correction et mise à jour de sécurité, version par version.",
        "de": "Release Notes für %(name)s — jede Änderung, jeder Fix und jedes Sicherheitsupdate, Version für Version.",
        "es": "Notas de versión de %(name)s — cada cambio, corrección y actualización de seguridad, versión a versión.",
        "it": "Note di rilascio di %(name)s — ogni modifica, fix e aggiornamento di sicurezza, versione per versione.",
    },
    "Privacy policy for %(name)s — what data we store (only ciphertext and hashed metadata), what we never collect (your messages, your keys), and how long anything is kept.": {
        "nl": "Privacybeleid van %(name)s — welke data we opslaan (alleen ciphertext en gehashte metadata), wat we nooit verzamelen (je berichten, je sleutels) en hoe lang we iets bewaren.",
        "fr": "Politique de confidentialité de %(name)s — quelles données nous stockons (uniquement du chiffré et des métadonnées hachées), ce que nous ne collectons jamais (vos messages, vos clés), et combien de temps tout est conservé.",
        "de": "Datenschutzerklärung für %(name)s — welche Daten wir speichern (nur Ciphertext und gehashte Metadaten), was wir nie erfassen (deine Nachrichten, deine Schlüssel) und wie lange wir was aufbewahren.",
        "es": "Política de privacidad de %(name)s — qué datos almacenamos (solo cifrado y metadatos con hash), qué no recogemos nunca (tus mensajes, tus claves) y cuánto tiempo se conserva todo.",
        "it": "Informativa sulla privacy di %(name)s — quali dati memorizziamo (solo ciphertext e metadati hash), cosa non raccogliamo mai (i tuoi messaggi, le tue chiavi) e per quanto tempo conserviamo qualcosa.",
    },
    "%(name)s uses essential cookies only — session, language and theme preference. No tracking, analytics or third-party assets.": {
        "nl": "%(name)s gebruikt alleen essentiële cookies — sessie, taal en thema-voorkeur. Geen tracking, analytics of externe assets.",
        "fr": "%(name)s n'utilise que des cookies essentiels — session, langue et préférence de thème. Pas de suivi, pas d'analyse, pas d'assets tiers.",
        "de": "%(name)s verwendet nur essenzielle Cookies — Sitzung, Sprache und Theme-Präferenz. Kein Tracking, keine Analyse, keine Drittanbieter-Assets.",
        "es": "%(name)s usa solo cookies esenciales — sesión, idioma y preferencia de tema. Sin seguimiento, analítica ni recursos de terceros.",
        "it": "%(name)s usa solo cookie essenziali — sessione, lingua e preferenza tema. Niente tracciamento, analytics o asset di terze parti.",
    },
    "Security policy and threat model for %(name)s — encryption model, account hardening, and how to report vulnerabilities.": {
        "nl": "Beveiligingsbeleid en dreigingsmodel voor %(name)s — versleutelingsmodel, account-hardening en hoe je kwetsbaarheden meldt.",
        "fr": "Politique de sécurité et modèle de menace de %(name)s — modèle de chiffrement, durcissement des comptes et comment signaler des vulnérabilités.",
        "de": "Sicherheitsrichtlinie und Bedrohungsmodell für %(name)s — Verschlüsselungsmodell, Konto-Härtung und wie du Schwachstellen meldest.",
        "es": "Política de seguridad y modelo de amenazas de %(name)s — modelo de cifrado, endurecimiento de cuentas y cómo reportar vulnerabilidades.",
        "it": "Policy di sicurezza e threat model di %(name)s — modello di crittografia, hardening dell'account e come segnalare vulnerabilità.",
    },
